...
#else
	strcpy_s(mem_leak_info->mem_info.file_name,FILE_NAME_LENGTH,alloc_info.file_name);
#endif	 
 ...
 */
void * xrealloc(void *mem_address, unsigned int newsize,const char * file, unsigned int line)
{
	void * ptr = realloc (mem_address,newsize);
	if (ptr != NULL) 
	{
		remove_mem_info(mem_address);
		add_mem_info(ptr, newsize, file, line);
	}
	return ptr;
}
...
void report_mem_leak(void)
{
  ...
  fopen_s(&fp_write,OUTPUT_FILE, "wt");
	...
	if(fp_write != NULL)
	{
		...
		for(leak_info = ptr_start; leak_info != NULL; leak_info = leak_info->next)
		{
			sprintf_s(info,1024, "address : %d\n", leak_info->mem_info.address);
			fwrite(info, (strlen(info) + 1) , 1, fp_write);
			sprintf_s(info,1024, "size    : %d bytes\n", leak_info->mem_info.size);			
			fwrite(info, (strlen(info) + 1) , 1, fp_write);
			sprintf_s(info,1024, "file    : %s\n", leak_info->mem_info.file_name);
			fwrite(info, (strlen(info) + 1) , 1, fp_write);
			sprintf_s(info,1024, "line    : %d\n", leak_info->mem_info.line);
			fwrite(info, (strlen(info) + 1) , 1, fp_write);
			sprintf_s(info,1024, "%s\n", "-----------------------------------");	
			fwrite(info, (strlen(info) + 1) , 1, fp_write);
		}
